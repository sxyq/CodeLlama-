...
function normalizeParams(value: unknown, fallback: TaskParams): TaskParams {
  ...
  return {
    ...
    negative_prompt: typeof value.negative_prompt === 'string' ? value.negative_prompt : fallback.negative_prompt,
    num_inference_steps: value.num_inference_steps === null || (typeof value.num_inference_steps === 'number' && Number.isFinite(value.num_inference_steps))
      ? value.num_inference_steps
      : fallback.num_inference_steps,
    seed: value.seed === null || (typeof value.seed === 'number' && Number.isFinite(value.seed))
      ? value.seed
      : fallback.seed,
  }
}
