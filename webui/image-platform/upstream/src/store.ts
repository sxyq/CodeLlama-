  ...
  persist(
    (set, get) => ({
      ...
      setMaskDraft: (maskDraft) => {
        // 当前模型不支持 mask 编辑，明确提示而不进入编辑流程
        if (maskDraft) {
          useStore.getState().showToast('Mask editing unsupported by current model.', 'error')
          return
        }
        set((s) => {
          const inputImages = orderImagesWithMaskFirst(s.inputImages, undefined)
          return syncActiveInputDraft(s, {
            maskDraft,
            inputImages,
